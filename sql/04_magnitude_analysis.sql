...
--------------------------------------------------------------
-- 3. Top 10 Employees by Experience who are Active 
--------------------------------------------------------------
SELECT TOP 10
    employee_id,
    name,
...
FROM dbo.employee_analytics
WHERE status = 'Active'
ORDER BY experience_years DESC, hire_date DESC;
...
SELECT
    ...
    RANK() OVER (ORDER BY AVG(CAST(salary AS BIGINT)) DESC) AS dept_salary_rank
FROM dbo.employee_analytics
GROUP BY department;
